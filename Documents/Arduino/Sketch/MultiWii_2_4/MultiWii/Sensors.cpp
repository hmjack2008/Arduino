...
// ************************************************************************************************************
// I2C Accelerometer KIONIX KXCJ9 1008 
// ************************************************************************************************************
#if defined(KXCJ9)
#if !defined(KXCJ9_ADDRESS)
  #define KXCJ9_ADDRESS 0xF
#endif

void ACC_init () {
  delay(10);
  i2c_writeReg(KXCJ9_ADDRESS,0x1B,0xC0);
}

void ACC_getADC () {
  i2c_getSixRawADC(KXCJ9_ADDRESS,0x06);

  ACC_ORIENTATION( ((int8_t(rawADC[1])<<8) | int8_t(rawADC[0]))>>4 ,
                   ((int8_t(rawADC[3])<<8) | int8_t(rawADC[2]))>>4 ,
                   ((int8_t(rawADC[5])<<8) | int8_t(rawADC[4]))>>4 );
  ACC_Common();
}
#endif
...
// ************************************************************************************************************
// I2C Gyroscope and Accelerometer LSM9DS1 (9DOF)
// by hmjack2003@163.com (2015/09/11)
// ************************************************************************************************************
#if defined(LSM9DS1)
#if !defined(LSM9DS1_AG_ADDRESS)
  // #define LSM9DS1_AG_ADDRESS(sa0) ((sa0) == 0 ? 0x6A : 0x6B) // 6B
  #define LSM9DS1_AG_ADDRESS    0x6B // address pin5 SD0_AG low (VCC)
#endif
#if !defined(LSM9DS1_M_ADDRESS)
  // #define LSM9DS1_M_ADDRESS(sa1)  ((sa1) == 0 ? 0x1C : 0x1E) // 1E
  #define LSM9DS1_M_ADDRESS     0x1E // address pin6 SD0_M low (VCC)
#endif
...
//  xgWriteByte(ORIENT_CFG_G, tempRegValue);
  delay(50);
  //i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x10,0B01000000); // CTRL_REG1_G (0x10)  gyro odr and bw 
  i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x10,0B10011000); // CTRL_REG1_G (0x10)  Gyroscope:+/-2000 dps  ODR:238Hz BW:14Hz
  delay(10);
  i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x12,0B01000000); // CTRL_REG3_G (0x12)  High-pass filter enable.
  delay(10);  
  i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x1E,0B00111000); // CTRL_REG4 (0x1E)  enable Gyro
  delay(50);
}

void Gyro_getADC () {
  i2c_getSixRawADC(LSM9DS1_AG_ADDRESS, 0x18); // OUT_X_L_G (0x18), OUT_X_H_G (0x19)
  GYRO_ORIENTATION( ((rawADC[1]<<8) | rawADC[0]) ,  // OUT_X_L_G (0x18), OUT_X_H_G (0x19)
				    ((rawADC[3]<<8) | rawADC[2]) ,  // OUT_Y_L_G (0x1A), OUT_Y_H_G (0x1B)
				    ((rawADC[5]<<8) | rawADC[4]) ); // OUT_Z_L_G (0x1C), OUT_Z_H_G (0x1D)
  GYRO_Common();
}
...
//  xgWriteByte(CTRL_REG7_XL, tempRegValue); 
  delay(50);
  i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x1F,0B00111000); // CTRL_REG5_XL (0x1F)  enable accelerometer
  delay(10);
  i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x20,0B10010000); // CTRL_REG6_XL (0x20)  ODR:238Hz full-scale:+/-4g
  delay(10);
  i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x21,0B10000000); // CTRL_REG7_XL (0x21)  High resolution mode for accelerometer enable 
  delay(50);  
  //i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x10,0B01000000); // CTRL_REG1_G (0x10)  gyro odr and bw 
  //i2c_writeReg(LSM9DS1_AG_ADDRESS, 0x10,0B10011000); // CTRL_REG1_G (0x10)  Gyroscope:+/-2000 dps  ODR:238Hz BW:14Hz
  //delay(50);
...
void ACC_getADC () {
  i2c_getSixRawADC(LSM9DS1_AG_ADDRESS, 0x28); // OUT_X_L_XL (0x28), OUT_X_H_XL (0x29)
  ACC_ORIENTATION( ((rawADC[1]<<8) | rawADC[0])>>3 ,  // OUT_X_L_XL (0x28), OUT_X_H_XL (0x29)
				   ((rawADC[3]<<8) | rawADC[2])>>3 ,  // OUT_Y_L_XL (0x2A), OUT_Y_H_XL (0x2B)
				   ((rawADC[5]<<8) | rawADC[4])>>3 ); // OUT_Z_L_XL (0x2C), OUT_Z_H_XL (0x2D)
  ACC_Common();
}
...
//  mWriteByte(CTRL_REG5_M, tempRegValue);
  delay(50);
  i2c_writeReg(LSM9DS1_M_ADDRESS,0x20,0B11010000);  // CTRL_REG1_M (0x20) Continuous conversion mode
  delay(10);  
  i2c_writeReg(LSM9DS1_M_ADDRESS,0x22,0B00000000);  // CTRL_REG3_M (0x22) X Y-axis operative High-performance, 10Hz
  delay(10);  
  i2c_writeReg(LSM9DS1_M_ADDRESS,0x23,0B00001000);  // CTRL_REG4_M (0x23) Z-axis operative High-performance   
  delay(50);
}

void Device_Mag_getADC() {
i2c_getSixRawADC(LSM9DS1_M_ADDRESS,0x28); // OUT_X_L_M (0x28), OUT_X_H_M (0x29)
MAG_ORIENTATION( ((rawADC[1]<<8) | rawADC[0]) ,  // OUT_X_L_M (0x28), OUT_X_H_M (0x29)
				 ((rawADC[3]<<8) | rawADC[2]) ,  // OUT_X_L_M (0x2A), OUT_X_H_M (0x2B)
				 ((rawADC[5]<<8) | rawADC[4]) ); // OUT_X_L_M (0x2C), OUT_X_H_M (0x2D)
}
#endif
